...
import axios, { baseUrlCentral } from "../../utils/axios/index";
...
import Central from "../../resources/images/central.png";
...
function Index(props) {
  ...
  useEffect(() => {
    ...
    return () => {
      localStorage.removeItem("edges_list_central");
      localStorage.removeItem("node_central");
      clearInterval(intervalId);
    };
  }, []);

  useEffect(() => {
    document.title = "Central Flagship";
    setLoading(true);
    (async () => {
      axios
        .get(`${baseUrlCentral}/topInterfaceUtilizations`)
        .then((response) => {
      ...
      await axios
        .get(`${baseUrlCentral}/get-routers`)
        .then((response) => {
          setLoading(false);
          ...
          setEdges(edges_list);
          localStorage.setItem("node_central", convertJSONToString(node_list));
        })
        .catch((error) => {
          setLoading(false);
          console.error("Error fetching data: ", error);
    ...
    axios
      ...
      .then((response) => {
        ...
        for (let i = 0; i < edges_list.length; i++) {
          ...
          edges_list[i].label = "";
          // edges_list[i].labelPosition = "left";
  ...
  const handleOnEdgeMouseEnter = (event, edge) => {
    let edgesString = localStorage.getItem("edges_list_central");
    let edgeData = [];
    if (edgesString) {
