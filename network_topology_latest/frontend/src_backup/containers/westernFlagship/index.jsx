...
import axios, { baseUrlWestern } from "../../utils/axios/index";
...
import Western from "../../resources/images/western.png";
...
function Index(props) {
  ...
  useEffect(() => {
    ...
    return () => {
      localStorage.removeItem("edges_list_western");
      localStorage.removeItem("node_western");
      clearInterval(intervalId);
    };
  }, []);

  useEffect(() => {
    document.title = "Western Flagship";
    setLoading(true);
    (async () => {
      axios
        .get(`${baseUrlWestern}/topInterfaceUtilizations`)
        .then((response) => {
      ...
      await axios
        .get(`${baseUrlWestern}/get-routers`)
        .then((response) => {
          setLoading(false);
          ...
          setEdges(edges_list);
          localStorage.setItem("node_western", convertJSONToString(node_list));
        })
        .catch((error) => {
          setLoading(false);
          console.error("Error fetching data: ", error);
  ...
  const handleOnEdgeMouseEnter = (event, edge) => {
    let edgesString = localStorage.getItem("edges_list_western");
    let edgeData = [];
    if (edgesString) {
